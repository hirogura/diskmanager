...
```
 /opt/diskmanager/
  ├── server.py            # Web サーバー本体（ポート 3361、バージョン 0.7.0）
...
- パーティション操作（グラフィカル表示。サイズ拡大／縮小、作成、削除、ドライブ内の全パーティション削除、前方移動、表からのマウント／アンマウント（一時的・fstab永続）。マウント中・システムパーティションは保護。ただしbtrfsはオンライン拡縮小のみ可。システムドライブ上の空き領域への作成、非システム領域（未マウント）のリサイズ・削除・移動・前方拡大は可。ext4・btrfs・xfs作成時は所有者の一般ユーザー化（既定ON）・chmod 777開放（既定OFF）を選択可）
- S.M.A.R.T.情報表示
